...
void i2c_rep_start(uint8_t address) {
  ...
  waitTransmissionI2C((1<<TWINT) | (1<<TWEN));              // wait until transmission completed
}
...
// I2C Compass MMC5883
// ************************************************************************************************************
// I2C adress: 0x30 (8bit)   0x30 (7bit)
// ************************************************************************************************************
...
#define MAG_CTRL_REG1 0x08
#define MMC_STATUS 0x07

void Mag_init() {
  i2c_writeReg(MAG_ADDRESS,MAG_CTRL_REG1,0x01);
}


static void getADC() {
  i2c_writeReg(MAG_ADDRESS,MMC_STATUS,0x00);
  i2c_getSixRawADC(MAG_ADDRESS,MAG_DATA_REGISTER);
  MAG_ORIENTATION( ((rawADC[1]<<8) | rawADC[0]) ,
                   ((rawADC[3]<<8) | rawADC[2]) ,
                   ((rawADC[5]<<8) | rawADC[4]) );
